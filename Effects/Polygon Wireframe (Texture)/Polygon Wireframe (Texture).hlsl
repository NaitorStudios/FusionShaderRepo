...
PS_OUTPUT ps_main(in PS_INPUT In)
{
    ...
    float4 _Polygon = Fun_Polygon(In.texCoord);
        ...
        float _Alpha = saturate(
            step(_UV.x, _Width) + 
            step(1.0 - _Width, _UV.x) + 
            step(_UV.y, _Height) + 
            step(1.0 - _Height, _UV.y) +
            step(abs(_UV.x + _UV.y - 1.0), max(_Width, _Height) * 2.0)
        );

    float4 _Render_Texture = S2D_Image.Sample(S2D_ImageSampler, _UV) * In.Tint * _Alpha;
...
PS_OUTPUT ps_main_pm(in PS_INPUT In)
{
    ...
    float4 _Render_Texture = Demultiply(S2D_Image.Sample(S2D_ImageSampler, _UV)) * In.Tint * _Alpha;
