...
PS_OUTPUT ps_main(in PS_INPUT In)
{
    ...
    float _Border = all(_In >= 0.0) && all(_In <= 1.0);
         ...
         float4 _Render = float4(float3(_Color.rgb), S2D_Image.Sample(S2D_ImageSampler, _In).a * In.Tint.a);
...
PS_OUTPUT ps_main_pm(in PS_INPUT In)
{
    ...
    float _Border = all(_In >= 0.0) && all(_In <= 1.0);
         ...
         float4 _Render = Demultiply(float4(float3(_Color.rgb), S2D_Image.Sample(S2D_ImageSampler, _In).a * In.Tint.a));
