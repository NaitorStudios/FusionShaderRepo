...
PS_OUTPUT ps_main( float2 In : TEXCOORD0 ) {
	...
	/* This part is weird, I know, but it's the only way to make it work.
	If you try to use "break;" to stop the loop, you get an error "unexpected token "break"".
	If you try to set i to 67 to stop the loop, suddenly the number of instruction slots used goes through the roof for some reason.
	So, the loop just has to run the full number of times, which apparently isn't that bad on a GPU.
	Also, replacing step.x with something else, like a simple "done" integer, increases the instruction slots used by a lot as well.
	*/
	for ( int i = 0; i < 67; i++) {
		if ( step.x < 10000 ) {
			ray += step;
	...
	/* For DX11, I would expect to say something like:
	int ground = 0;
	for ( int i = 0; i < 67; i++) { // hopefully we can use a much higher number than 67 in DX11
		ray += step;
		if ( tex2D( imgHeightmap, ray.xz ).r * fYScale > ray.y ) {
			ground = 1;
			break;
