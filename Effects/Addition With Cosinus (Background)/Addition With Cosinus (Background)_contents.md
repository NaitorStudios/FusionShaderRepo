## Info
**Shader Packs:** Foxioo Shader Pack

**License:** MIT

**Last Updated:** 2026-01-22 14:20  

## Shader files
- **Addition With Cosinus (Background).fx** — DirectX9 source code
- **Addition With Cosinus (Background).hlsl** — DirectX11 source code
- **Addition With Cosinus (Background).fxc** — DirectX11 compiled
- **Addition With Cosinus (Background).premultiplied.fxc** — DirectX11 compiled (Premultiplied*)
- **Addition With Cosinus (Background).fxao** — Android source code
- **Addition With Cosinus (Background).fxmo** — iOS/Mac source code
*Sometimes a shader needs Premultiplied version to fix Alpha Channel handling in Premultiplied graphics.*  