...
float4 ps_main( float4 TexCoord : TEXCOORD0 ) : COLOR0
{
	...
	float4 cornerA = tex2D(Tex0, samplecoord + float2(corner_offset.x,0));
	float4 cornerB = tex2D(Tex0, samplecoord + float2(0,corner_offset.y));

	//retrieve opposite colors
	float4 oppositeA = tex2D(Tex0, samplecoord - float2(corner_offset.x,0));
	float4 oppositeB = tex2D(Tex0, samplecoord - float2(0,corner_offset.y));
