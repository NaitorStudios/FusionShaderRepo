...
PS_OUTPUT ps_main( in PS_INPUT In )
{
    ...
    if(iH==0 || (iH==1 && In.texCoord.x >fX) || (iH==2 && In.texCoord.x <fX) ) {
    	if(iV==0 || (iV==1 && In.texCoord.y >fY) || (iV==2 && In.texCoord.y <fY) ) {
		    float holeRGB = pow(max(0,min(1,sqrt(pow(In.texCoord.y-fY,2)/fRatio+pow(In.texCoord.x-fX,2)*fRatio)/fD)),fE)*fC;
		    if(iInvert==0) Out.Color.rgb *= 1-holeRGB;
		    else Out.Color.rgb *= holeRGB;
		  }
    }
    return Out;
}
