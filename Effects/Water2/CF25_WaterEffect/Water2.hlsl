...
PS_OUTPUT ps_main( in PS_INPUT In )
{
    ...
    float3 normal = ( Demultiply(sNormalMap.Sample( sNormalMapSampler, coordNormalMap )).xyz * (float3)2.0 ) - (float3)1.0;
    ...
    float4 col = Demultiply(sBaseTexture.Sample( sBaseTextureSampler, coordBase ) );
    float4 ref = Demultiply(sReflectionMap.Sample( sReflectionMapSampler, coordReflectionMap ));
	...
	float alpha = Demultiply(sBaseTexture.Sample( sBaseTextureSampler, In.texCoord )).a;
	Out.Color.a = lerp(0, Out.Color.a,alpha);
	Out.Color *=  In.Tint;
    return Out;
}
