...
float4 ps_main( in float2 In : TEXCOORD0 ) : COLOR0 {
  ...
  int cr = 0;
  float cH = ( ( ( ( fH % 360.0 ) + 360.0 + 90.0 ) % 360.0 ) );
  float cV = ( ( ( ( fV % 360.0 ) + 360.0 + 90.0 ) % 360.0 ) );
  ...
  if (cr % 2 == 0) {outColor = tex2D( img, xy);}
  else {outColor = tex2D( iR, float2(1.0 - xy.x, xy.y));}
