...
Texture2D<float4> rubyTexture : register(t0);
sampler rubyTextureSampler : register(s0);
...
#define TEX2D(c) pow(abs(rubyTexture.Sample(rubyTextureSampler, (c))), inputGamma)        		
...
float4 ps_main( in PS_INPUT In ) : SV_TARGET
{
	...
	float2 rubyTextureSize = 1.0 / float2(fPixelWidth,fPixelHeight);
	
	float2 rubyInputSize = rubyTextureSize * float2(inputW,inputH);
	float2 rubyOutputSize = rubyTextureSize * float2(outputW,outputH);
		
	// The size of one texel, in texture-coordinates.
	float2 one = 1.0 / rubyTextureSize;
	
	// Texture coordinates of the texel containing the active pixel				
	//float2 xy = radialDistortion(In.texCoord);

	float2 coord = In.texCoord * (rubyTextureSize / rubyInputSize);
	float2 cc = coord - 0.5;
	float dist = dot(cc, cc) * distortion;				
	float2 xy = (coord + cc * (1.0 + dist) * dist) * rubyInputSize / rubyTextureSize;
	...
	float4 col  = clamp(0.0, coeffs.x * TEX2D(xy + float2(-one.x, 0.0))   + coeffs.y * TEX2D(xy) + coeffs.z * TEX2D(xy + float2(one.x, 0.0)) + coeffs.w * TEX2D(xy + float2(2.0 * one.x, 0.0)), 1.0);
	float4 col2 = clamp(0.0, coeffs.x * TEX2D(xy + float2(-one.x, one.y)) + coeffs.y * TEX2D(xy + float2(0.0, one.y)) + coeffs.z * TEX2D(xy + one)              + coeffs.w * TEX2D(xy + float2(2.0 * one.x, one.y)), 1.0);
	...
	float mod_factor = In.texCoord.y * rubyOutputSize.y * rubyTextureSize.y / rubyInputSize.y;
	...
	return float4(mul_res, 1.0) * In.Tint;
}
...
#define TEX2DPM(c) Demultiply(TEX2D(c))
...
float4 ps_main_pm( in PS_INPUT In ) : SV_TARGET
{
	...
	// scanlines at the horizontal location of the current pixel,
	// using the Lanczos coefficients above.								
	float4 col  = clamp(0.0, coeffs.x * TEX2DPM(xy + float2(-one.x, 0.0))   + coeffs.y * TEX2DPM(xy) + coeffs.z * TEX2DPM(xy + float2(one.x, 0.0)) + coeffs.w * TEX2DPM(xy + float2(2.0 * one.x, 0.0)), 1.0);
	float4 col2 = clamp(0.0, coeffs.x * TEX2DPM(xy + float2(-one.x, one.y)) + coeffs.y * TEX2DPM(xy + float2(0.0, one.y)) + coeffs.z * TEX2DPM(xy + one)              + coeffs.w * TEX2DPM(xy + float2(2.0 * one.x, one.y)), 1.0);
