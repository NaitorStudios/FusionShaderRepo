...
float4 ps_main( float4 TexCoord : TEXCOORD0 ) : COLOR0
{
	...
	float4 A = tex2D(Tex0, samplecoord + mul(rot, float2(-1,-1))*PixelSize );
	float4 B = tex2D(Tex0, samplecoord + mul(rot, float2( 0,-1))*PixelSize );
	float4 C = tex2D(Tex0, samplecoord + mul(rot, float2( 1,-1))*PixelSize );

	float4 D = tex2D(Tex0, samplecoord + mul(rot, float2(-1, 0))*PixelSize);
	float4 E = tex2D(Tex0, samplecoord);  //in the original Scale3X algorithm, "E" denotes the center pixel which we are expanding
	float4 F = tex2D(Tex0, samplecoord + mul(rot, float2( 1, 0))*PixelSize);

	//G unused
	float4 H = tex2D(Tex0, samplecoord + mul(rot, float2( 0, 1))*PixelSize );
	//I unused
